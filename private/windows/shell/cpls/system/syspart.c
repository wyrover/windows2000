...

{
    ...
    if(!b) {
        ...
        if(GetLastError() == ERROR_INSUFFICIENT_BUFFER) {
            ...
            DriveLayoutSize += 1024;
            if(p = REALLOC(DriveLayout,DriveLayoutSize)) {
                DriveLayout = p;
            } else {
                GotIt = FALSE;
                goto c2;
    ...
    for(Drive=TEXT('A'); Drive<=TEXT('Z'); Drive++) {
        ...
        if(MyGetDriveType(Drive) == DRIVE_FIXED) {
            ...
            if(QueryDosDevice(DriveName,Buffer,sizeof(Buffer)/sizeof(TCHAR))) {
                ...
                if (memcmp(Buffer,TEXT("\\device\\harddisk"),16*sizeof(TCHAR))) {
                    hardDiskNumber = QueryHardDiskNumber(Drive);
                    if (hardDiskNumber != (DWORD) -1) {
                        wsprintf(Buffer, TEXT("\\device\\harddisk%d"),
                                 hardDiskNumber);
                    }
                }
    ...
    )
{
    ...
    if(ISNT()) {
        ...
        if(!NtOpenSymLinkRoutine || !NtQuerSymLinkRoutine) {
