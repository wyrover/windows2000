...
//
//  When you click on cancel, wDeltaDateTime is subtracted from
//  the current values.  You cannot just revert to what it was when it
//  started as time continues and dates could change.
//
    ...
    int nIndex)
{
    ...
    {
        //
        //  Delta is previous delta + (users setting versus system time)
        //
    ...
    int iMonth)
{
    ...
    if (wDateTime[MONTH] != iMonth)
    {
        ...
        //
        if (wDateTime[DAY] > GetDaysOfTheMonth(iMonth))
        {
            wDateTime[DAY] = GetDaysOfTheMonth(iMonth);
        }

        wDeltaDateTime[MONTH] += iMonth - wDateTime[MONTH];
        wPrevDateTime[MONTH] = wDateTime[MONTH] = iMonth;
    ...
    HDC hDC)
{
    ...
    GetCharWidth(hDC, TEXT('0'), TEXT('9'), nNumWidth);
    ...
    BOOL bDate)
{
    ...
    else
    {
        ...
        else
        {
            ...
            GetProfileString(szIntl, TEXT("s1159"), IntlDef.s1159, sz1159, 8);
            GetProfileString(szIntl, TEXT("s2359"), IntlDef.s2359, sz2359, 8);
        }
        i = 0;
    }
    ...
    HWND hDlg)
{
    ...
    if (g_Modified)
    {
        lpSystemTime->wHour   = g_Time[HOUR];
        lpSystemTime->wMinute = g_Time[MINUTE];
        lpSystemTime->wSecond = g_Time[SECOND];
    }
    else
    {
#ifdef WIN32
    ...
    LPARAM lParam)
{
    ...
    GetProfileString(szIntl, TEXT("sTime"), IntlDef.sShortDate, szShortDate, 12);
    ...
    SendMessage(hwndScroll, UDM_SETBUDDY, (int)GetDlgItem(hDlg, DATETIME_YEAR), 0L);
    ...
    switch (uMsg)
    {
...
#else
                return ((BOOL)DefWindowProc(hDlg, WM_CTLCOLOREDIT, wParam, lParam));
#endif
            }
            return (0);
            break;
        ...
        case ( WM_NOTIFY ) :
        {
            ...
            switch (((NMHDR *)lParam)->code)
            {
                ...
                case ( PSN_APPLY ) :
                {
                    wDateTime[MINUTE] = g_Time[MINUTE];
                    wDateTime[SECOND] = g_Time[SECOND];

                    if (g_b24HR)
                    {
                        wDateTime[HOUR] = g_Time[HOUR];
                    }
                    else
                    {
                    ...
                    switch (GET_WM_COMMAND_CMD(wParam, lParam))
                    {
                        case ( EN_CHANGE ) :
                        {
                            ...
                            if (GET_WM_COMMAND_ID(wParam, lParam) == DATETIME_YEAR && wDateTime[MONTH] == 2)
                            {
                                ...
                                {
                                    wDeltaDateTime[DAY] += g_sDateInfo[DAY].nMax - wDateTime[DAY];
                                    wDateTime[DAY] = g_sDateInfo[DAY].nMax;
                                    SetDate();
                                    GetDate();
                                }
                                InvalidateRect( GetDlgItem(hDlg, DATETIME_CALENDAR),
    ...
    PTZINFO zone)
{
    if (zone || state->zone)
    {
        ...
        EnableWindow(GetDlgItem(page, IDD_AUTOMAGIC), dayval);
    ...
    TZPAGE_STATE *state)
{
    ...
    {
        EnableWindow(GetDlgItem(hDlg, IDD_AUTOMAGIC), FALSE);
    }
...
#ifdef WINNT
    if ((bAutoMagicEnabled == 0) ||
        (bAutoMagicTimeChange == 0) ||
...
BOOL CALLBACK TimeZoneDlgProc(
    HWND hDlg,
    UINT uMsg,
    WPARAM wParam,
    LPARAM lParam)
{
    TZPAGE_STATE *state = (TZPAGE_STATE *)GetWindowLong(hDlg, DWL_USER);
    ...
    switch (uMsg)
    {
        case ( WM_INITDIALOG ) :
        {
            ...
            SetWindowLong(hDlg, DWL_USER, (LONG)state);
                    ...
                    switch (((NMHDR *)lParam)->code)
                    {
                        case ( PSN_APPLY ) :
                        {
                            ...
                            SetTheTimezone(
                                ...
                                IsWindowEnabled(GetDlgItem(hDlg, IDD_AUTOMAGIC)),
                                GetPTZ(hDlg, -1) );
    ...
    LPCTSTR cmdline)
{
    ...
    apsp[0].pszTemplate = MAKEINTRESOURCE(DLG_DATETIME);
