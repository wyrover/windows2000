                ...
                LPCTSTR FAR dirs[])
{
    ...
    LoadString(hInstance, idLoadHigh, szCommand, ARRAYSIZE(szCommand));
...
void SetUpMouse(LPWIZDATA lpwd, int i)
{
    ...
    LoadString(hInstance, IDS_MOUSEENV, szMouseEnv, ARRAYSIZE(szMouseEnv));
...
{
    if (lpwd->DosOpt[i].dwStdOpt = RMOPT_MOUSE)
    {
...
void ConfOptInit(HWND hDlg, LPPROPSHEETPAGE lpp)
{
    ...
    himlState = ImageList_LoadImage(hInstance, MAKEINTRESOURCE(IDB_CHECKSTATES), 0, 2,
    ...
    *lpszBootDir = 0;
    ...
    if (*lpszBootDir == 0)
    {
        ...
        if (lpszBootDir[0] != TEXT('\0')
            ...
            LoadString(hInstance, IDS_DEFBOOTDIR, lpszBootDir, cchBootDir);
    }
}
...
void ItemChanged(LPWIZDATA lpwd, LPNM_LISTVIEW lpnmlv)
{
        ...
        (!(lpnmlv->uOldState & LVIS_FOCUSED)))
    {
        ...
        GetOptVal(lpwd, lvi.lParam, c_szRegDosOptTip, szTip, sizeof(szTip));
...
void _inline NoSupportMsg(LPWIZDATA lpwd, int iOpt)
{
    ...
    if (lpszDesc)
    {
        ...
        if (lpszMsg)
        {
            ...
            LoadString(hInstance, IDS_NOSUPPORT1, lpszMsg, 512);
            LoadString(hInstance, IDS_NOSUPPORT2, lpszMsg+lstrlen(lpszMsg), 256);
...
void CopyEnvironment(LPTEXTDATA lpAE)
{
    ...
    LoadString(hInstance, IDS_SETCMD, szSetCmd, ARRAYSIZE(szSetCmd));
...
PIFWIZERR SetConfOptions(LPWIZDATA lpwd, HWND hwndOptions, BOOL fForceCleanCfg)
{
    ...
    LoadString(hInstance, IDS_CSLOWSTR, szCSLowCmd, ARRAYSIZE(szCSLowCmd));
            ...
            switch(lpnm->code)
            {
                ...
                case PSN_SETACTIVE:
                    ...
                    if (lpwd->dwFlags & WDFLAG_PIFPROP)
                    {
                        ...
                        LoadString(hInstance, IDS_OK, szOK, ARRAYSIZE(szOK));
