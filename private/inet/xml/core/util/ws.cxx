...
static WSPAGEINFO * s_pWSPages = NULL;
static LONG_PTR s_cPages = 0;
...
DWORD AddModuleSections(HANDLE hProcess, PVOID pBase, WSPAGEENTRY **ppPE)
{
    ...
    for (i=0; i<cSections; i++, pish++, (*ppPE)++)
    {
        ...
        (*ppPE)->addrEnd = (DWORD_PTR)pBase + (DWORD_PTR)pish->VirtualAddress + (DWORD_PTR)pish->SizeOfRawData - 1;
...
HRESULT FillPETable(HANDLE hProcess)
{
    ...
    pPE->addrEnd = (DWORD_PTR)mbi.BaseAddress + (DWORD_PTR)mbi.RegionSize;
...
HRESULT WsTakeSnapshot(HANDLE hProcess)
{
    ...
    cPages = ((DWORD_PTR)psvmctr.WorkingSetSize / (DWORD_PTR)si.dwPageSize)*2 + 1;

    delete [] s_pWSPages;
    s_cPages = 0;
	pWSPages = s_pWSPages = new WSPAGEINFO[(int)cPages];
	if (!pWSPages)
    {
        hr = E_OUTOFMEMORY;
	    goto Cleanup;
    ...
    s_cPages = cPages = *((DWORD_PTR *)pWSPages++);
    ...
    for (i=0; i<cPages; i++, pWSPages++)
    {
        for (j=0; j<s_cPageEntries; j++)
        {
            ...
            {
                VirtualQuery((void *)(*(DWORD *)pWSPages & 0xFFFFF000), &mbi, sizeof(mbi));
                ...
                {
                    s_aPE[j].dwUncommited += si.dwPageSize;
                }
                if (s_aPE[j].addrEnd == 0xFFFFFFFF)
                    pWSPages->fUnknown = 1;
                else
                    pWSPages->fUnknown = 0;
                break;
...
LONG_PTR WsTotal()
{
    ...
    {
        total += s_aPE[i].dwSize;
    }

    return total;
}

LONG_PTR WsStart(LONG_PTR row)
{
    if (row >= 0 && (DWORD_PTR)row < s_cPageEntries)
        return s_aPE[row].addrStart;
    else
        return 0;
}
...
bool WsPageInfo(LONG_PTR page, MEMORY_BASIC_INFORMATION * pmbi)
{
    if (page >=0 && page < s_cPages)
    {
        VirtualQuery((void *)(*(DWORD *)&s_pWSPages[page] & 0xFFFFF000), pmbi, sizeof(MEMORY_BASIC_INFORMATION));
        return s_pWSPages[page].fUnknown == true;
    }
    return false;
}
