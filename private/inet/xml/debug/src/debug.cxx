...
extern "C" {
    ...
    // BUGBUG - These are renamed so that we don't link in the ones from shlwapi.
...
TAG     tagDefault                  = tagNull;
TAG     tagError                    = tagNull;
TAG     tagWarn                     = tagNull;
TAG     tagAssertPop                = tagNull;
TAG     tagAssertExit               = tagNull;
TAG     tagAssertStacks             = tagNull;
TAG     tagTestFailures             = tagNull;
TAG     tagTestFailuresIgnore       = tagNull;
TAG     tagRRETURN                  = tagNull;
TAG     tagValidate                 = tagNull;
TAG     tagLeaks                    = tagNull;
TAG     tagLeaksExpected            = tagNull;
TAG     tagSymbols                  = tagNull;
TAG     tagSpySymbols               = tagNull;
TAG     tagTrackItf                 = tagNull;
TAG     tagTrackItfVerbose          = tagNull;
TAG     tagThrd                     = tagNull;
TAG     tagMemoryStrict_            = tagNull;
TAG     tagCoMemoryStrict_          = tagNull;
TAG     tagMemoryStrictTail_        = tagNull;
TAG     tagMemoryStrictAlign_       = tagNull;
TAG     tagOLEWatchvar              = tagNull;
TAG     tagMemTrace                 = tagNull;
TAG     tagPerf_                    = tagNull;
TAG     tagTraceCalls               = tagNull;
...
BOOL    SendTagToDisk(TAG tag, BOOL fSendToDisk);
...
DllThreadAttach()
{
    ...
    if (!pts)
    {
        Assert((INT_PTR)"Debug Thread initialization failed");
...
DllProcessAttach(HINSTANCE hinst)
{
    ...
    g_ataginfo[] =
    {
        ...
        &tagSymbols,                tgtyOther,  "!Symbols",
            "Stacktraces & symbols for leaks",                  FALSE,
        &tagSpySymbols,             tgtyOther,  "!Symbols",
            "Stacktraces & symbols for IMallocSpy leaks",       FALSE,
    ...
    if (GetEnvironmentVariableA("MSXML_LOGPATH", szLogPath, MAX_PATH))
    {
        OpenLogFile(szLogPath);
    }
...
DllProcessDetach(void)
{
    ...
    // Communicate to a client app (like omtest.exe) that there was a memory leak.
    char achMem[32];
    if (GetEnvironmentVariableA("MSXML_DRT", achMem, sizeof(achMem)))
    {
        achMem[0] = g_fMemoryLeak ? '1' : '0';
        achMem[1] = 0;
        BOOL f = SetEnvironmentVariableA("MSXML_MEMORYLEAK", achMem);
        Assert(f);
    }
...

{
    ...
    if ( pszSlash )
    {
        StrCpyA( szDebugFile, pszSlash+1 );
    ...
    if (hfile != INVALID_HANDLE_VALUE)
    {
        for (;;)
        {
            ...
            {
                if (ptgrc->TestFlag(TGRC_FLAG_VALID) &&
                    !StrCmpA(rgchOwner, ptgrc->szOwner) &&
                    !StrCmpA(rgchDescrip, ptgrc->szDescrip))
                {
...
FWriteDebugState( CHAR * szDebugFile )
{
    ...
    if (hfile != INVALID_HANDLE_VALUE)
    {
        for (tag = tagMin; tag < tagMac; tag++)
        {
            ...
            Assert((INT_PTR)ptgrc->szOwner);
            Assert((INT_PTR)ptgrc->szDescrip);
...
#else
        Assert(rgch[cch - 4] == '.');
        StrCpyA(&rgch[cch - 4], szStateFileExt);
#endif
#else
...
TAG FindTag(char * szTagDesc)
{
    Assert((INT_PTR)szTagDesc);
    for (TAG i = tagMin; i < tagMac; i++)
    {
        if (mptagtgrc[i].szDescrip && strlen(mptagtgrc[i].szDescrip))
        {
            if (!StrCmpIA(mptagtgrc[i].szDescrip, szTagDesc))
            {
                return i;
            }
        }
        ...
        BOOL    fEnabled)
{
            ...
            tag++, ptgrc++)
    {
        if (ptgrc->TestFlag(TGRC_FLAG_VALID))
        {
            if(!(StrCmpA(szOwner, ptgrc->szOwner) ||
                StrCmpA(szDescrip, ptgrc->szDescrip)))
            {
                return tag;
            }
        }
    ...
    Assert((INT_PTR)szOwner);
    Assert((INT_PTR)szDescrip);
    cb = strlen(szOwner) + 1;
    ...
    StrCpyA(szDescripDup, szDescrip);
...
//    if (ptgrc->TestFlag(TGRC_FLAG_DISK))
    ...
    {
       // This doesn't appear to do anything, except yank in more of the CRT. simonb(06-17-1998)
#ifndef BUILD_XMLDBG_AS_LIB
       printf(achBuf);
#else
...
void
TaggedTraceCallers(TAG tag, int iStart, int cTotal)
{
    ...
    for (i = 0; i < c; i++)
    {
        ...
        TaggedTraceEx(tag, TAG_NONAME, "%s", achSymbol);
    }
}
...
OpenLogFile(LPCSTR szFName)
{
...
#else
            Assert(rgch[cch - 4] == '.');
            StrCpyA(&rgch[cch - 4], szDbgOutFileExt);
#endif
#else
            CHAR   achAppLoc[MAX_PATH];
            DWORD   dwRet;
    ...
    else
    {
        ...
        StrCpyNA(rgch, szFName, sizeof(rgch) / sizeof(rgch[0]) - 1);
        rgch[sizeof(rgch) / sizeof(rgch[0]) - 1] = 0;

        if(szFName[nLen - 1] == '\\')
        {
            StrCatA(rgch, szDbgOutFileName);
        }
    ...
    {
        char msg[200];
        wnsprintfAX(msg, sizeof(msg) / sizeof(msg[0]), "Cannot create the log file \"%s\"" SZ_NEWLINE, rgch);
