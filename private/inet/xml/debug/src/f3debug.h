...
#define Assert(x)       do { if (!((DWORD)((INT_PTR)(x))|g_dwFALSE) && AssertImpl(__FILE__, __LINE__, #x))\
                               F3DebugBreak(); } while (g_dwFALSE)

#define AssertSz(x, sz) do { if (!((DWORD)(x)|g_dwFALSE) && AssertImpl(__FILE__, __LINE__, sz))\
                               F3DebugBreak(); } while (g_dwFALSE)

#define AssertLocSz(x, File, Line, sz)   do { if (!((DWORD)(x)|g_dwFALSE) && AssertImpl(File, Line, sz))\
                               F3DebugBreak(); } while (g_dwFALSE)
#define IF_DBG(x) x
#define WHEN_DBG(x) x
#define IF_NOT_DBG(x)
...
typedef int TAG;
    ...
    int  __cdecl PerfDbgLogFn(TAG tag, void * pvObj, CHAR * szFmt, ...);
    ...
    #define tagCoMemoryStrict       TagCoMemoryStrict()
    #define tagMemoryStrictTail     TagMemoryStrictTail()
    ...
    TAG EXPORT WINAPI TagCoMemoryStrict(void);
    TAG EXPORT WINAPI TagMemoryStrictTail(void);
...
//--------------------------------------------------------------------------
    ...
    size_t  EXPORT WINAPI DbgPreAlloc(size_t cb);
    void *  EXPORT WINAPI DbgPostAlloc(void *pv);
    void *  EXPORT WINAPI DbgPreFree(void *pv);
    void    EXPORT WINAPI DbgPostFree(void);
    size_t  EXPORT WINAPI DbgPreRealloc(void *pv, size_t cb, void **ppv);
    void *  EXPORT WINAPI DbgPostRealloc(void *pv);
    void *  EXPORT WINAPI DbgPreGetSize(void *pv);
    size_t  EXPORT WINAPI DbgPostGetSize(size_t cb);
    void *  DbgPreDidAlloc(void *pv);
    BOOL    DbgPostDidAlloc(void *pv, BOOL fActual);

    void    EXPORT WINAPI DbgRegisterMallocSpy(void);
    void    EXPORT WINAPI DbgRevokeMallocSpy(void);
    void    EXPORT WINAPI DbgMemoryTrackDisable(BOOL fDisable);
    void    EXPORT WINAPI DbgCoMemoryTrackDisable(BOOL fDisable);
    void    EXPORT WINAPI DbgMemoryBlockTrackDisable(void *pv);

    void    EXPORT WINAPI TraceMemoryLeaks(void);
    BOOL    EXPORT WINAPI ValidateInternalHeap(void);
    void    EXPORT WINAPI DbgDumpProcessHeaps();
    void    EXPORT WINAPI DbgDumpMemory();
    size_t  EXPORT WINAPI DbgTotalAllocated();
    void *  EXPORT WINAPI PointerRequestFromActual(void *);

    extern BOOL CheckSmallBlockHeap();

    void * EXPORT __cdecl DbgMemSetName(void *pv, char * szFmt, ...);
    char * EXPORT WINAPI DbgMemGetName(void * pv);

#ifdef WIN16
#define IsVirtualEnabled()  FALSE
#define CHECK_HEAP()
#else

    extern BOOL IsVirtualEnabled();

    //
    // Use the CHECK_HEAP macro to do thorough heap validation.
    //
    #define CHECK_HEAP()    Assert(ValidateInternalHeap() && CheckSmallBlockHeap() && "Corrupted heap!")
#endif
...
void        MtAdd(ULONG_PTR mt, LONG lCnt, LONG lVal);
void         PerfClear();
