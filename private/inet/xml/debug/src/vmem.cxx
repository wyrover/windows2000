...
VMemQueryProtect(void * pv, DWORD cb)
{
    ...
    return(mbi.RegionSize == cb ? (mbi.Protect ? mbi.Protect : mbi.AllocationProtect) : PAGE_GUARD);
}
...
VMemIsValid(void * pv)
{
    ...
    if (VMemQueryProtect((BYTE *)pvmi + PAGE_SIZE * 2, (ULONG)(pvmi->cbFill1 + pvmi->cb + pvmi->cbFill2)) != PAGE_READWRITE)
    {
    ...
    if (VMemQueryProtect((BYTE *)pv + pvmi->cbFill1 + pvmi->cb + pvmi->cbFill2, PAGE_SIZE) != PAGE_NOACCESS)
    {
...
VMemAlloc(size_t cb, DWORD dwFlags, void * pvUser)
{
    ...
    Assert((BOOL)(ULONG_PTR)VMemIsValid((PVOID)(ULONG_PTR)pvmi->pv));	
