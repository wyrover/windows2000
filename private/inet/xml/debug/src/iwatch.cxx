...
{
    return DbgPostAlloc(malloc(DbgPreAlloc(cb))); 
}
...
CTrack::CTrack(IUnknown *pUnk, REFIID iid, char *pch, BOOL fTrackOnQI)
{
    ...
    DbgMemSetName(this, "TI %s p=%08x ssn=%d id=%s", pch, _pUnk, _ulSSN, GetIIDName(iid));
}
...
CWrapArg::CWrapArg(CTrack *pTrack, REFIID iid, void **ppv)
{
    ...
    {
        (*_ppUnk)->AddRef();
        DbgTrackItf(iid, pTrack->_pch, TRUE, ppv);
    }
}
...
void
DbgTrackItf(REFIID iid, char * pch, BOOL fTrackOnQI, void **ppv)
{
    
    if (     
           *ppv == NULL
        || !(IsTagEnabled(tagTrackItf) || IsTagEnabled(tagTrackItfVerbose)) 
        || iid == IID_IUnknown 
#if defined(_M_IX86)
