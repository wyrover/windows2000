...
#define INPUT_VARS(cinfo)  \
	...
	const JOCTET HUGEP * next_input_byte = datasrc->next_input_byte;  \
	unsigned long bytes_in_buffer = datasrc->bytes_in_buffer
...
#define ADOBE_LEN 12
  ...
  if (length >= ADOBE_LEN) {
    ...
    if (b[0]==0x41 && b[1]==0x64 && b[2]==0x6F && b[3]==0x62 && b[4]==0x65) {
      ...
      version = ((int) b[5] << 8) + b[6];
      flags0 =  ((int) b[7] << 8) + b[8];
      flags1 =  ((int) b[9] << 8) + b[10];
      transform = b[11];
