...

{
    ...
    successful = IopConcatenateUnicodeStrings( &unicodeKeyName,
                                               &tmpKeyName,
                                               ServiceKeyName);
    ...
    if (!IopFixupDeviceId(unicodeKeyName.Buffer)) {
...

{
    ...
    for (i = 0; i < count; i++) {
        ...
        if (physicalDeviceObject) {
            ...
            if (deviceNode && (IopIsDevNodeProblem(deviceNode, CM_PROB_DISABLED) || IopIsDevNodeProblem(deviceNode, CM_PROB_HARDWARE_DISABLED))) {
                ZwClose(handle);
        ...
        if (deviceFlags & CONFIGFLAG_DISABLED) {
            ...
            status = IopGetServiceInstanceCsConfigFlags( ServiceKeyName,
                                                         i,
                                                         &deviceFlags
                                                         );
...

{
    ...
    if (deviceObject) {
        deviceNode = (PDEVICE_NODE)deviceObject->DeviceObjectExtension->DeviceNode;
        if (deviceNode && (IopIsDevNodeProblem(deviceNode, CM_PROB_DISABLED) || IopIsDevNodeProblem(deviceNode, CM_PROB_DISABLED))) {
            enabled = FALSE;
            goto exit;
        }
    }
...

{
    ...
    if (!ARGUMENT_PRESENT(DeviceInstanceHandle)) {
        ...
        //
        // Treat HTREE\ROOT\0 specially
        //
        PiWstrToUnicodeString(&unicodeName, REGSTR_VAL_ROOT_DEVNODE);

        if (RtlEqualUnicodeString(&unicodeName, DeviceInstance, TRUE)) {

            deviceReference = IopRootDeviceNode->PhysicalDeviceObject;
        ...
        status = IopOpenRegistryKeyEx( &deviceHandle,
                                       handle,
                                       DeviceInstance,
                                       KEY_READ
                                       );
    ...
    *Length = 0;
    ...
    if (ResourceType == QUERY_RESOURCE_LIST) {
        ...
        if (Preference & REGISTRY_BOOT_CONFIG) {
            ...
            status = IopReadDeviceConfiguration( handle,
...
Routine Description:
    ...
    Called directly from IopProcessNewDeviceNode, and indirecly via IopDeviceNodeCapabilitiesToRegistry
...

{
    ...
    PiWstrToUnicodeString(&unicodeName, REGSTR_VAL_UI_NUMBER);
    tmpValue = Capabilities->UINumber;
    if(tmpValue != (ULONG)-1) {
        ZwSetValueKey(handle,
                      &unicodeName,
                      TITLE_INDEX_VALUE,
...
Routine Description:
    ...
    This is a callback routine to IopApplyFunctionToSubkeys, that gets called
    ...
    IopApplyFunctionToSubkeys, with instructions to delete all enumerated
...
--*/
{
    ...
    //
    status = IopApplyFunctionToSubKeys(
        KeyHandle,
        NULL,
        KEY_ALL_ACCESS,
