...
#define DsoPrint(x) Print("%.*s", nIndent, "                      "); Print x
...
*  actual field offset from the beginning of the structure
*
    ...
    BOOL fOneField)
{
    ...
    while (psot->pszField != NULL) {
        ...
        if (pAddress == NULL) {
            ...
            if (!fBF) {
                int width = 8;
                if (dwSize == sizeof(BYTE) || ((BYTE)pdwValue & 1)) {
                    dwValue &= 0xff;
                    width = 2;
                } else if (dwSize == sizeof(WORD) || ((BYTE)pdwValue & 2)) {
                    dwValue &= 0xffff;
                    width = 4;
                }
...
BOOL Idso(DWORD opts, LPSTR pszCmdLine)
{
    ...
    DWORD dwSize;
    LPSTR pszField, pszAddress, pszRepeat;
    PSTRUCTURESTABLE pst;
    PSTRUCTUREOFFSETSTABLE psot;
    PVOID pAddress = NULL;
    DWORD_PTR nRepeat = 1;
    ...
    if (*pszField != 0) {
        ...
        if (psot == NULL) {
            ...
            fOneField = TRUE;
            pszRepeat = dsoTerminateString(pszAddress, &dwSize);
        ...
        if ((*pszRepeat != 0) && (*pszRepeat == '*')) {
            nRepeat = (DWORD_PTR)EvalExp(pszRepeat+1);
            if (nRepeat == 0) {
                /*
                 * EvalExp displayed the error message, so return silently
                 */
    ...
    while (nRepeat--) {
        ...
        pAddress = (PBYTE)pAddress + pst->dwSize;
    }
