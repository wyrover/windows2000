...

{
    ...
    if (Accessible) {
        if  (MmDbgWriteCheck((PVOID)Address, &Opaque) != NULL) {
    ...
    for (Index = 0; Index < BREAKPOINT_TABLE_SIZE; Index += 1) {
        if (KdpBreakpointTable[Index].Flags &
            ...
            if ((KdpBreakpointTable[Index].Address >= (PVOID)GLOBAL_BREAKPOINT_LIMIT) ||
                ...
                //
                if (MmDbgWriteCheck((PVOID)KdpBreakpointTable[Index].Address,                                       &Opaque) == NULL) {
                    KdpOweBreakpoint = TRUE;
                ...
                MmDbgReleaseAddress(
                        (PVOID)KdpBreakpointTable[Index].Address,
                        Opaque
                        );
