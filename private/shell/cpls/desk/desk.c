...
//  { DLG_MONITOR,      DisplayPageProc,     REGSTR_VAL_DISPCPL_NOSETTINGSPAGE   },
};
...
//static const TCHAR sc_szRegDisplayHandlers[] = REGSTR_PATH_CONTROLSFOLDER TEXT("\\Display") TEXT("\\shellex\\PropertySheetHandlers");
//static const TCHAR sc_szRegDesktopHandlers[] = REGSTR_PATH_CONTROLSFOLDER TEXT("\\Desktop") TEXT("\\shellex\\PropertySheetHandlers");
...
void DeskShowPropSheet( HINSTANCE hInst, HWND hwndParent, LPCTSTR cmdline )
{
    ...
    //
    // move OEM registry handlers around
    //
...
HWND FAR PASCAL CreateCoverWindow( DWORD flags )
{
    HWND hwndCover = CreateWindowEx( gdwCoverStyle,
                                     sc_szCoverClass, g_szNULL, WS_POPUP | WS_VISIBLE | flags, 0, 0,
                                     GetSystemMetrics( SM_CXSCREEN ), GetSystemMetrics( SM_CYSCREEN ),
                                     NULL, NULL, hInstance, NULL );
    if( hwndCover )
    {
