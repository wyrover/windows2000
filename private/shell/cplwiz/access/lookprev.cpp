...
LONG CALLBACK CLookPrev::LookPreviewWndProc (HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	PAINTSTRUCT ps;

	CLookPrev *pThis = (CLookPrev *)GetWindowLong(hWnd, GWL_USERDATA);
	if(!pThis)
	{
...
void CLookPrev::Draw(HDC hdc)
{
...
#else
	{
		...
		DrawText(hdc, lpszText, lstrlen(lpszText), &rcT, DT_VCENTER | DT_SINGLELINE | DT_EXPANDTABS);
