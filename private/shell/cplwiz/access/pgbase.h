...
//			_ASSERTE(FALSE); // we could not find the range
	...
	void Display()
	{
		TCHAR sz[1000];
		TCHAR szTemp[50];
		wsprintf(sz, TEXT("Elements = %i\r\n"), m_nCount);
		for(int i=0;i<m_nCount;i++)
		{
			wsprintf(szTemp, TEXT("%i\r\n"), m_rgdwPageIds[i]);
			lstrcat(sz, szTemp);
		}
		MessageBox(NULL, sz, TEXT("Wizard Pages"), MB_OK);
	}
...
protected:
	...
	//
	static BOOL DlgProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam);
...
protected:
	...
	{
		AdjustWizPageOrder();
		SetWindowLong(hwnd, DWL_MSGRESULT, sm_WizPageOrder.GetPrevPage(m_dwPageId));
		return TRUE;
	}
	virtual LRESULT OnPSN_WizNext(HWND hwnd, INT idCtl, LPPSHNOTIFY pnmh)
	{
		AdjustWizPageOrder();
		SetWindowLong(hwnd, DWL_MSGRESULT, sm_WizPageOrder.GetNextPage(m_dwPageId));
		return TRUE;
	}
