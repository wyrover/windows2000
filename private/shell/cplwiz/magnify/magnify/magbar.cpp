...
{
	if (TrackText()) OnEventXMove(wParam, lParam, FALSE);
	return(0);
}
...
{
	if (TrackFocus()) OnEventXMove(wParam, lParam, FALSE);
	return(0);
}
...
{
	if (TrackCursor()) OnEventXMove(wParam, lParam, TRUE);
	return(0);
}
...
BOOL CMagBar::OnInitDialog()
{
	...
	DirectDrawCreate(NULL, &m_IDirectDraw, NULL);

	// Set the initial state of the appbar.
	SetState(abs);

	// Set the system screen reader flag on so that apps
	// like Word 97 will expose the caret position.
/*JMC: INVESTIGATE*/	::SystemParametersInfo(SPI_SETSCREENREADER, TRUE, NULL, NULL);
// NEED INI BROADCAST
...
void CMagBar::OnDestroy()
{
	...
	// JMR: Remove?:    gfAppExiting=TRUE;
/*JMC: INVESTIGATE*/	::SystemParametersInfo(SPI_SETSCREENREADER, FALSE, NULL, NULL);
/*JMC: INVESTIGATE*/	::SendMessage(HWND_BROADCAST, WM_SETTINGCHANGE, SPI_SETSCREENREADER, FALSE);

	if(m_IDirectDraw)
		m_IDirectDraw->Release();
...
//	UpdateDIBColorTable(m_dcOffScreen, pdc->GetSafeHdc(), NULL);
	...
	m_dcOffScreen.BitBlt(0, 0, sizeZoomRect.cx, sizeZoomRect.cy,
		&dcScreen, ptZoomRectTopLeft.x, ptZoomRectTopLeft.y,
	...
	{
		CPoint ptIconDest;
		ptIconDest.x = m_ptZoom.x - ptZoomRectTopLeft.x - ii.xHotspot;
		ptIconDest.y = m_ptZoom.y - ptZoomRectTopLeft.y - ii.yHotspot;
		// blt the upper half of the mask to the screen
		m_dcMem.SelectObject(CBitmap::FromHandle(ii.hbmMask));
	...
	if(m_IDirectDraw)
		m_IDirectDraw->WaitForVerticalBlank(DDWAITVB_BLOCKBEGIN, NULL);
...
//		g_pGetCursorInfo(&ci);
	...
	if(bShowCursor)
	{
		// blt the upper half of the mask to the screen
		m_dcMem.SelectObject(CBitmap::FromHandle(ii.hbmMask));
		m_dcIconBuffer.BitBlt(0, 0, 32, 32, &dcScreen, rcIconSrc.left, rcIconSrc.top, InvertColors() ? NOTSRCCOPY : SRCCOPY);
		m_dcIconBuffer.BitBlt(0, 0, 32, 32, &m_dcMem, 0, 0, SRCAND);
...
VOID CMagBar::DrawZoomRect()
{
	...
	rc.SetRect(x - m_cxZoomed / 2, y - m_cyZoomed / 2, rc.left + m_cxZoomed, rc.top + m_cyZoomed);
	rc.InflateRect(1, 1);
	CClientDC dcScreen(NULL);
...
VOID CMagBar::CopyToClipboard()
{
	...
	if (OpenClipboard())
	{
		...
		GetClientRect(&rc);
		CBitmap bm;
		VERIFY(bm.CreateCompatibleBitmap(&dcSrc, rc.Width(), rc.Height()));
		CDC dcDst;
		VERIFY(dcDst.CreateCompatibleDC(&dcSrc));
		...
		VERIFY(::SetBitmapDimensionEx(bm,
			(rc.Width()  * dwMM10PERINCH) / (DWORD) dcSrc.GetDeviceCaps(LOGPIXELSX), 
			(rc.Height() * dwMM10PERINCH) / (DWORD) dcSrc.GetDeviceCaps(LOGPIXELSY), 
			NULL));

		VERIFY(NULL != dcDst.SelectObject(&bm));
		VERIFY(dcDst.BitBlt(0, 0, rc.Width(), rc.Height(), &dcSrc, rc.left, rc.top, 
			InvertColors() ? NOTSRCCOPY : SRCCOPY));

		VERIFY(EmptyClipboard());
		VERIFY(NULL != SetClipboardData(CF_BITMAP, bm));
...
BOOL CMagBar::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
	...
	if(HTCLIENT == nHitTest || (fPrimaryMouseBtnDown && HTCAPTION))
	{
		...
		::SetCursor(AfxGetApp()->LoadCursor(IDC_CUSTOM_HAND));
		return TRUE;
	}
