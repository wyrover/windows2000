...
LPDWORD GetMRUValue(HKEY hkeySubKey, LPCTSTR pszRegValue)
{
    ...
    // Get the size
    if (ERROR_SUCCESS == SHQueryValueEx(hkeySubKey, pszRegValue, NULL, NULL, NULL, &cbVal))
    {
        ...
        if (pResult)
        {
            ...
            if (ERROR_SUCCESS != SHQueryValueEx(hkeySubKey, pszRegValue, NULL, NULL, (LPBYTE) DATAPDATAEX(pResult), &cbVal))
            {
...
HRESULT ImportOldMRU(HKEY hkeySubKey, LPDWORD pOrder, LPDWORD pcbVal)
{
    ...
    if (ERROR_SUCCESS == SHQueryValueEx(hkeySubKey, (LPTSTR)szMRUEX_OLD, NULL, NULL, (LPBYTE)szOldMRU, &cbOldMRUStr))
    {
        DWORD dwIndex = 0;
        TCHAR szOldIndex[2];
...
HANDLE CreateMRUListLazyEx(LPMRUINFO lpmi, const void FAR *lpData, UINT cbData, LPINT lpiSlot)
{
    ...
    if (SHQueryValueEx(hkeySubKey, (LPTSTR)szMRUEX, NULL, &dwType, (LPBYTE)pOrder, &cbVal) == ERROR_SUCCESS)
    {
...
NotInRegistry:
    ...
    for (pTemp = pOrder, pNewOrder = pMRU->cOrder; *pTemp != (DWORD)-1; ++pTemp)
    {
        ...
        if (lpmi->fFlags & MRU_BINARY) {
            ...
            cbVal = 0;
            // first find the size
            if ((SHQueryValueEx(hkeySubKey, szTemp, NULL, &dwType, NULL, &cbVal)
            ...
            // now really get it
            DATASIZE(pVal) = cbVal;
            if (SHQueryValueEx(hkeySubKey, szTemp, NULL, &dwType, pVal+sizeof(DWORD),
