...
struct MEMINFO {
    ...
    int     iLine;              // line where memory block was allocced
};
...
void *  __cdecl operator new( unsigned int nSize, LPCSTR pszFile, const int iLine )
{
    LPVOID pv; 
...
void *  __cdecl operator new( unsigned int nSize )
{
    ...
    TraceMsg(TF_FTPALLOCS, "operator new() pv=%#08lx, nSize=%u, caller=%#08lx.", pv, nSize, pstack[2]);
    add_to_memlist( 0, pv, nSize, DBGMEM_UNKNOBJ, "UNKNOWN", pstack[2] );
    ...
    )
{
    if ( hMem )
    {
        ...
        memset( hMem, 0xfe, LocalSize( hMem ));
    }
...
BOOL GetLeakDetectionFunctionTable(LEAKDETECTFUNCS *pTable)
{
    ...
    if(pTable)
    {
        ...
        pTable->pfnDumpMemLeak = _DumpMemLeak;
        pTable->pfnDebugMemLeak = _DebugMemLeak;
